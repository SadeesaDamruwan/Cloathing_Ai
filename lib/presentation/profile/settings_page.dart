...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text("Settings", style: TextStyle(fontWeight: FontWeight.w900)), backgroundColor: Colors.transparent),
          ...
          children: [
            ...
            _buildButton(_isSaving ? "Saving..." : "Save Details", () async {
              ...
              await _db.updateUserProfile(user!.uid, {
                'age': _ageCtrl.text, 'gender': _genderCtrl.text, 'height': _heightCtrl.text, 'weight': _weightCtrl.text
              });
              setState(() => _isSaving = false);
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Profile Updated!")));
            }),
            const SizedBox(height: 30),
            _buildButton("Log Out", () => _auth.signOut()),
  ...
  Widget _buildInput(String label, TextEditingController ctrl) {
    return Column(
      ...
      children: [
        ...
        NeumorphicContainer(
          isInner: true,
          child: TextField(controller: ctrl, decoration: const InputDecoration(border: InputBorder.none, contentPadding: EdgeInsets.symmetric(horizontal: 15))),
        ),
  ...
  Widget _buildButton(String label, VoidCallback onTap) {
    return GestureDetector(
      ...
      child: NeumorphicContainer(
        child: SizedBox(height: 55, width: double.infinity, child: Center(child: Text(label, style: const TextStyle(fontWeight: FontWeight.bold)))),
      ),
    );
  }
  ...
  Widget _buildDeleteButton() {
    return GestureDetector(
      ...
      child: NeumorphicContainer(
        ...
        child: const SizedBox(height: 55, width: double.infinity, child: Center(child: Text("Delete Account", style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)))),
      ),
    );
  }
